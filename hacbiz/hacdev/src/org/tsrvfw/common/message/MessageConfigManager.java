...
public class MessageConfigManager {
	...
	public static String getViewMessageFromInnerCode(String innerCode)
			throws TsrvfwSystemException {
		...
		if (targetMessageMap == null){
			throw new TsrvfwSystemException("指定された内部コードが定義されていません。内部コード:" + innerCode);
		}
		
		StringBuilder message = new StringBuilder();
		message.append((String) targetMessageMap.get(TAG_NAME_OUTER_CD));
		message.append(MESSAGE_SEPARATOR);
		message.append((String) targetMessageMap.get(TAG_NAME_VIEW_MESSAGE));
